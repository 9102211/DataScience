...
def coffee_lotto(student_list, target_num):
    target_list = []
#    s_list = random.sample(student_list,target_num) * 참고
#    print(s_list)
...
#    print(student_list)
    while len(random_list) < 10:
        ...
        if student_list[i] not in random_list:
            random_list.append(student_list[i])

    print(f"발표자 순서는 아래와 같습니다.\n{random_list}\n")
...
while True:
    ...
    if option == "1":
        ...
        student_list = ["김유진", "김지혜", "문성준", "박종민", "송지예", "양석훈", "이예지", "임성혁", "한권제", "현재봉"]
